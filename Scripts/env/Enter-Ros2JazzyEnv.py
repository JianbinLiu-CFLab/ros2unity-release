#!/usr/bin/env python3
"""
Prepare a clean ROS 2 Jazzy / R2FU build environment on Windows.
...
DEFAULT_ROS2_ROOT = workspace_root() / "ros2-windows" / "ros2_jazzy"
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run commands inside a clean ROS 2 Jazzy / R2FU Windows environment."
    )
...
def main(argv: list[str]) -> int:
    ...
    if not args.quiet:
        print("Starting cmd.exe with ROS 2 Jazzy environment. Type exit to return.")
